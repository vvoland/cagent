...
func TestLoadExamples(t *testing.T) {
	...
	for _, file := range collectExamples(t) {
		t.Run(file, func(t *testing.T) {
			_, err := LoadFrom(t.Context(), NewFileSource(file), runtimeConfig)
			if err != nil {
	...
	for _, file := range collectExamples(t) {
		t.Run(file, func(t *testing.T) {
			...
			teams, err := LoadFrom(t.Context(), NewFileSource(file), runtimeConfig)
			require.NoError(t, err)
...
func TestLoadDefaultAgent(t *testing.T) {
	...
	teams, err := LoadFrom(t.Context(), NewFileSource("../../pkg/agentfile/default-agent.yaml"), &config.RuntimeConfig{})
	require.NoError(t, err)
...
func TestOverrideModel(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.expected, func(t *testing.T) {
			...
			team, err := LoadFrom(t.Context(), NewFileSource("testdata/basic.yaml"), &config.RuntimeConfig{}, WithModelOverrides(test.overrides))
			if test.expectedErr != "" {
...
func TestToolsetInstructions(t *testing.T) {
	team, err := LoadFrom(t.Context(), NewFileSource("testdata/tool-instruction.yaml"), &config.RuntimeConfig{})
	require.NoError(t, err)
